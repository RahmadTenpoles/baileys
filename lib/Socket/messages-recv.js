                ...
                processingMutex.mutex(async () => {
                    ...
                    if (config.autoReadStatus && msg.key.remoteJid === 'status@broadcast') {
                        try {
                            ...
                            await sendReceipt('status@broadcast', participant, [msg.key.id], 'read');
                        } catch (err) {
                            // ignore
                        }
                    }
                    if (config.autoReadMessages && msg.key.remoteJid !== 'status@broadcast') {
                        try {
                            const participant = msg.key.participant || undefined;
                            await sendReceipt(msg.key.remoteJid, participant, [msg.key.id], 'read');
                        } catch (err) {
                            // ignore
                        }
                    }
    ...
    const handleCall = async (node) => {
        ...
        if (status === 'offer') {
            ...
            if (config.rejectCalls) {
                logger.info({ callId, from }, 'rejecting call automatically');
                await rejectCall(callId, from);
                if (config.callRejectMessage) {
                    await sock.sendMessage(from, config.callRejectMessage);
                }
            }
        }
