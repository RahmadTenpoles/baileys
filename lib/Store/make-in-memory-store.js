...
exports.default = (config) => {
    ...
    try {
        ...
        logger.error({ err }, 'failed to load persistent contacts cache');
    }
    ...
    const saveCache = () => {
        ...
        saveTimeout = setTimeout(() => {
            try {
                // fs.writeFileSync(cachePath, JSON.stringify(contacts, null, 2));
            } catch (err) {
                logger.error({ err }, 'failed to save persistent contacts cache');
            }
        }, 3000);
    };
