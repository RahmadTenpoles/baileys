...
const baileysVersion = [2, 3000, 1045649367];
const PLATFORM_MAP = {
    'aix': 'AIX',
    'darwin': 'Mac OS',
    'win32': 'Windows',
...
Object.assign(Browsers, BrowsersDict);
...
// CompanionWebClientType enum values (matches official WhatsApp Web pairing specs)
...
const getCompanionWebClientType = ([os, browserName]) => {
    ...
    return BROWSER_TO_COMPANION_WEB_CLIENT[browserName] || CompanionWebClientType.CHROME;
};
...
const generateMessageID = () => 'CLUTCH-' + (0, crypto_1.randomBytes)(6).toString('hex').toUpperCase();
...
const fetchLatestWaWebVersion = async (options = {}) => {
	try {
		...
		const fetchFn = typeof fetch !== 'undefined' ? fetch : fetch_1;
		const response = await fetchFn('https://web.whatsapp.com/sw.js', {
			...options,
			method: 'GET',
			headers
		});
		...
		if (!match || !match[1]) {
			return {
				version: baileys_version_json_1.version || baileysVersion,
				isLatest: false,
				error: { message: 'Client revision not found in sw.js' }
			};
		}
...
const fetchLatestBaileysVersion = async (options = {}) => {
    try {
        const waWeb = await fetchLatestWaWebVersion(options);
        if (waWeb.isLatest) {
            return waWeb;
        }
        return {
